...
bot.hears('O que verei no curso?', async ctx => {
    ...
    await ctx.reply('3. E você verá como eu fui feito, iso mesmo, você pode fazer uma cópia de mim.')
...
bot.hears('Como comprar o curso?', ctx => {
    ctx.replyWithMarkdown('Que bom... [link](https://www.cod3r.com.br)')
})
...
bot.on('location', async ctx => {
    try {
        ...
        const res = await axios.get(`${url}?lat=${lat}&lon=${lon}&APPID=065c8a33291c2f50a93bec0b5b3b5cd2&units=metric`)
        ...
        ctx.reply('Estou tendo problemas para pegar a tua localização. Você esta no planeta Terra? :P', tecladoOpcoes)
    }
